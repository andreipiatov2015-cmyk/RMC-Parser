...
public class FilterSession {
    ...
    /**
     * Человекочитаемая сводка активных фильтров вида "Подпись: значение(я)",
     * для показа в истории поиска. Сырые значения (id опций) переводятся в
     * их подписи через FilterDefinition, где это возможно.
     */
    public List<String> getActiveFiltersSummary() {
        ...
        for (Map.Entry<String, String> entry : singleValues.entrySet()) {
            ...
            String caption = resolveCaption(entry.getKey());
            String label = resolveOptionLabel(entry.getKey(), value);
            summary.add(caption + ": " + label);
        }
        ...
        for (Map.Entry<String, Set<String>> entry : multiValues.entrySet()) {
            ...
            String joined = String.join(", ", values.stream()
                    .map(v -> resolveOptionLabel(entry.getKey(), v))
                    .toList());
            summary.add(caption + ": " + joined);
        }
        
        return summary;
    }
    
    private String resolveCaption(String name) {
        FilterDefinition def = definitions.get(name);
        if (def != null && def.getCaption() != null && !def.getCaption().isEmpty()) {
            return def.getCaption();
        }
        return name;
    }
    
    private String resolveOptionLabel(String name, String value) {
        FilterDefinition def = definitions.get(name);
        if (def != null) {
            for (FilterOption option : def.getOptions()) {
