...
public class FilterParser {
    ...
    public static ParseResult parse(String html) {
        ...
        try {
            ...
            // Основной путь: проходим форму в реальном порядке DOM, разбивая
            // поля на секции по заголовкам-разделителям (.divider.header),
            // как на сайте. Структура секций не хардкодится — она целиком
            // определяется тем, что реально есть в HTML на момент парсинга,
            // поэтому появление/удаление/переименование разделов и полей на
            // сайте подхватывается автоматически при следующем запуске.
            List<FilterDefinition> filters = new ArrayList<>();
            List<FilterGroup> groups = new ArrayList<>();
            parseSectioned(filterForm, labelMap, filters, groups);
            
            // Фолбэк: если на странице нет .field-разметки (другая вёрстка,
            // либо тестовый HTML), парсим все select/input/textarea как
            // раньше — единым списком без секций.
            if (filters.isEmpty()) {
                filters.addAll(parseLooseElements(filterForm, labelMap));
                if (!filters.isEmpty()) {
                    groups.add(new FilterGroup.Builder()
                            .name("group_0")
    ...
    /**
     * Проходит форму в document-порядке (DFS), собирая .field-блоки в
     * секции по заголовкам-разделителям. Поля до первого разделителя
     * попадают в безымянную первую секцию (как на сайте).
     */
    private static void parseSectioned(Element filterForm, Map<String, String> labelMap,
                                        List<FilterDefinition> filters, List<FilterGroup> groups) {
        String currentTitle = null;
        List<FilterDefinition> currentGroupFilters = new ArrayList<>();
        int groupIndex = 0;
        
        for (Element el : filterForm.getAllElements()) {
            if (isDividerHeader(el)) {
                if (!currentGroupFilters.isEmpty()) {
                    groups.add(buildGroup(groupIndex++, currentTitle, currentGroupFilters));
                    currentGroupFilters = new ArrayList<>();
                }
            ...
            FilterDefinition filter = parseFieldElement(el, labelMap);
            if (filter != null) {
                filters.add(filter);
                currentGroupFilters.add(filter);
            }
        }
    ...
    private static boolean isFieldBlock(Element el) {
        return el.hasClass("field");
    }
    
    private static boolean isDividerHeader(Element el) {
        return el.hasClass("divider") && el.hasClass("header");
    }
    
    private static FilterGroup buildGroup(int index, String title, List<FilterDefinition> filters) {
        String resolvedTitle = (title == null || title.isEmpty()) ? "Общие параметры" : title;
        return new FilterGroup.Builder()
                .name("group_" + index)
                .title(resolvedTitle)
                .filters(filters)
                .build();
    ...
    /**
     * Разбирает один .field-блок: находит внутри него реальный элемент
     * управления (select/input/textarea, в т.ч. вложенный, как в .ui.calendar
     * у дат) и делегирует существующим парсерам по типу тега.
     */
    private static FilterDefinition parseFieldElement(Element fieldBlock, Map<String, String> labelMap) {
        Element control = fieldBlock.select("select, input, textarea").first();
        if (control == null) {
            return null;
        }
        return switch (control.tagName().toLowerCase()) {
            case "select" -> parseSelectElement(control, labelMap);
            case "textarea" -> parseTextareaElement(control, labelMap);
            default -> parseInputElement(control, labelMap);
        };
    }
    
    /**
     * Старое поведение парсера (до группировки по секциям): отдельные
     * проходы по select/input/textarea без учёта .field-разметки. Служит
     * фолбэком для страниц/тестового HTML без семантической разметки полей.
     */
    private static List<FilterDefinition> parseLooseElements(Element filterForm, Map<String, String> labelMap) {
        List<FilterDefinition> filters = new ArrayList<>();
    ...
    public static class ParseResult {
        ...
        /**
         * Фильтры, сгруппированные по секциям страницы (как на сайте:
         * "Учреждение", "Реестры", "Статусы в реестрах" и т.д.). Если
         * секции не удалось распознать, содержит одну группу со всеми
         * фильтрами.
         */
