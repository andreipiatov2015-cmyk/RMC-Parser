...
public class DjangoAuthenticationProvider {
    ...
    public DjangoAuthResult authenticate(String username, String password) {
        ...
        try {
            ...
            // Шаг 7: Проверяем успешность по наличию sessionid
            ...
            boolean hasSession = cookiesAfter.stream()
                    .anyMatch(name -> name.equalsIgnoreCase("sessionid") 
            ...
            if (hasSession) {
                ...
                return DjangoAuthResult.failure("Авторизация неуспешна: sessionid не получен", 
